...
export default function Costs() {
  ...
  function handleSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    addCost({
      ...
      type: fd.get('type') as 'fixo' | 'variavel',
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="stat-card">
          ...
          <p className="text-xl font-bold font-display">{fmt(fixedTotal)}</p>
        </div>
        <div className="stat-card">
          <p className="text-xs text-muted-foreground">Custos Variáveis</p>
          <p className="text-xl font-bold font-display">{fmt(varTotal)}</p>
        </div>
      </div>
          ...
          <TableBody>
            ...
            {costs.map(c => (
              <TableRow key={c.id}>
                ...
                <TableCell>{c.productId ? products.find(p => p.id === c.productId)?.name : '-'}</TableCell>
